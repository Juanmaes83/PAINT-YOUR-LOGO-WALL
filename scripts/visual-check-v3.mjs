...
async function forcePaint(i){await page.evaluate(i=>{const m=window.__PYLW_JOB_MANAGER__,j=m.jobs[i];m.paint(j,1,{size:54,edge:.68,wet:.78,forceFinal:true})},i);await page.waitForTimeout(80)}
async function hash(i){return page.evaluate(i=>{const j=window.__PYLW_JOB_MANAGER__.jobs[i],d=j.out.getContext('2d').getImageData(0,0,j.out.width,j.out.height).data;let h=2166136261;for(let p=0;p<d.length;p+=97){h^=(d[p]+d[p+1]*3+d[p+2]*7+d[p+3]*11)&255;h=Math.imul(h,16777619)}return h>>>0},i)}
...
try{
 ...
 const engineHashes={};for(const method of ['grass','particles','liquid','pixel','glitch']){await setMethod(0,method);await forcePaint(0);engineHashes[method]=await hash(0);const snap=await read();if(snap.jobsDetail[0].lastEffect?.engine!=='native-v3-canvas-engine')throw new Error(`${method} did not use V3 engine`)}if(new Set(Object.values(engineHashes)).size<5)throw new Error(`Effect engines did not produce distinct outputs ${JSON.stringify(engineHashes)}`);
 await page.screenshot({path:'artifacts/v3-five-engines.png',fullPage:true});
 ...
 const report={url,methods,engineButtons,engineHashes,motion:{grass:m1,grassWhileParticles:m1During2,particles:m2,glitch:m3},jobsDetail:restarted.jobsDetail,errors};fs.writeFileSync('artifacts/v3-report.json',JSON.stringify(report,null,2));console.log('V3 QA PASS',JSON.stringify(report,null,2));
