...
// Grass and Particles are now routed to source-faithful Escaparates Pro adapters.
const GRASS_PALETTE=[
...
void grassLegacy;void particlesLegacy;
...
const ENGINES={grass:renderGrassAdapter,particles:renderParticlesAdapter,liquid,pixel,glitch};
...
export function effectEngineSnapshot(method){if(method==='grass')return grassAdapterSnapshot();if(method==='particles')return particlesAdapterSnapshot();return{method,engine:ENGINES[method]?'temporary-v3-engine':'legacy-technique',live:!!ENGINES[method]}}
