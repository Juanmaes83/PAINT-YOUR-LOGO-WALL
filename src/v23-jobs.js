...
import {STYLE_PROFILES,drawTechniqueMask} from './v3-profile-extension.js';
import {renderV3Effect,V3_EFFECT_METHODS,effectEngineSnapshot} from './v3-effect-engine.js';
...
export class JobManager{
  ...
  create({name='Untitled',source=null,media=null,type='image',fileBlob=null,mediaStatus='ready',posterMedia=null,crew='aya',textConfig=null}={}){
    ...
    const method='brush',job={id:this.uid++,name,type,media,fileBlob,mediaStatus,posterMedia,source:sourceCanvas,mask,effect,out,temp,texture,mesh,scale:1.1,height:0,xOffset:0,rotation:0,duration:10,fit:'contain',living:true,method,reveal:STYLE_PROFILES[method].reveal,crew:crew||'aya',progress:0,baseY:.95,activated:false,activationCount:0,playFailures:0,activationDelay:.5,mediaError:null,textConfig,effectFrames:0,sourceFrame:0,lastEffect:effectEngineSnapshot(method)};this.jobs.push(job);this.layout();return job;
  }
  ...
  resetPlayback(){this.jobs.forEach(j=>{j.progress=0;j.activated=false;j.activationCount=0;j.playFailures=0;j.effectFrames=0;j.sourceFrame=0;if(j.type==='video')resetVideo(j)})}
  ...
  drawMedia(job){if(job.type!=='video'||!job.media||!job.activated||job.media.readyState<2)return false;drawFit(job.source.getContext('2d'),job.media,1024,512,job.fit);job.sourceFrame=(job.sourceFrame||0)+1;return true}
  compose(job,p,{size=42,edge=.34,wet=.22,forceFinal=false}={}){
    p=clamp(p);job.progress=p;if(job.type==='video'&&job.activated)this.drawMedia(job);const finalP=forceFinal?1:p;
    const t=job.temp.getContext('2d');t.clearRect(0,0,1024,512);
    if(V3_EFFECT_METHODS.has(job.method)){
      const time=job.type==='video'&&job.media?Number(job.media.currentTime||0):performance.now()/1000;
      renderV3Effect(job.effect,job.source,{method:job.method,progress:finalP,time,intensity:wet,size,edge});
      t.drawImage(job.effect,0,0);job.effectFrames=(job.effectFrames||0)+1;job.lastEffect=effectEngineSnapshot(job.method);
    }else{
      drawTechniqueMask(job.mask,{progress:p,method:job.method,size,edge});const mg=job.mask.getContext('2d');const completion=forceFinal?1:clamp((p-.86)/.14);if(completion>0){mg.save();mg.globalAlpha=ease(completion);mg.fillStyle='#fff';mg.fillRect(0,0,job.mask.width,job.mask.height);mg.restore()}t.drawImage(job.source,0,0);t.globalCompositeOperation='destination-in';t.drawImage(job.mask,0,0);t.globalCompositeOperation='source-over';job.lastEffect=effectEngineSnapshot(job.method);
    }
    edgeBlend(t,1024,512);const o=job.out.getContext('2d');o.clearRect(0,0,1024,512);o.globalAlpha=.88+wet*.10;o.drawImage(job.temp,0,0);o.globalAlpha=1;job.texture.needsUpdate=true;
  }
  paint(job,p,opts={}){this.compose(job,p,opts)}
  refreshLiveVideos(opts={}){let changed=0;for(const j of this.jobs){if(j.type==='video'&&j.activated&&j.mediaStatus==='live'&&j.media?.readyState>=2){this.compose(j,1,{...opts,forceFinal:true});changed++}}return changed}
  ...
  async restore(config){let j;if(config.type==='text')j=this.createText(config.textConfig||{text:config.name});else if(config.fileBlob)j=await this.fromFile(config.fileBlob);else return null;Object.assign(j,{name:config.name,scale:config.scale,height:config.height,xOffset:config.xOffset,rotation:config.rotation,duration:config.duration,fit:config.fit,living:config.living,method:STYLE_PROFILES[config.method]?config.method:'brush',reveal:config.reveal,crew:config.crew||'aya',activationDelay:config.activationDelay??.5,textConfig:config.textConfig||j.textConfig});if(j.type==='text')renderTextCanvas(j.source,j.textConfig);this.layout();return j}
}
