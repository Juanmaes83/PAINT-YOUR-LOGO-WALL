...
export class JobManager{
  ...
  create({name='Untitled',source=null,media=null,type='image'}={}){
    ...
    const method='brush',job={id:this.uid++,name,type,media,source:sourceCanvas,mask,out,temp,texture,mesh,scale:1,height:0,xOffset:0,rotation:0,duration:9,fit:'contain',living:true,method,reveal:STYLE_PROFILES[method].reveal,progress:0,baseY:.95};this.jobs.push(job);this.layout();return job;
  }
  layout(mode=this.layoutMode){this.layoutMode=mode;const n=Math.max(this.jobs.length,1);this.jobs.forEach((j,i)=>{let x=0,y=j.baseY+j.height;if(mode==='grid'){const cols=Math.ceil(Math.sqrt(n)),r=Math.floor(i/cols),c=i%cols;x=(c-(cols-1)/2)*2.45;y=1.65-r*1.45+j.height}else if(mode==='hero'){x=i===0?0:(i-(n)/2)*1.8;y=i===0?1.25:0.15+j.height;j.scale=i===0?Math.max(j.scale,1.18):Math.min(j.scale,.8)}else if(mode==='staggered'){x=(i-(n-1)/2)*2.15;y=.75+(i%2)*1.05+j.height}else if(mode==='mural'){x=(i-(n-1)/2)*1.72;y=.95+Math.sin(i*1.7)*.35+j.height}else{const span=Math.min(9.4,Math.max(0,(n-1)*2.25));x=n===1?0:-span/2+i*(span/Math.max(1,n-1));y=j.baseY+j.height}j.x=x+j.xOffset;j.mesh.position.set(j.x,y,-.16);j.mesh.scale.setScalar(j.scale);j.mesh.rotation.z=THREE.MathUtils.degToRad(j.rotation)})}
  ...
  paint(job,p,{size=42,edge=.34,wet=.22}={}){p=clamp(p);job.progress=p;this.drawMedia(job);drawTechniqueMask(job.mask,{progress:p,method:job.method,size,edge});const t=job.temp.getContext('2d');t.clearRect(0,0,1024,512);t.drawImage(job.source,0,0);t.globalCompositeOperation='destination-in';t.drawImage(job.mask,0,0);t.globalCompositeOperation='source-over';const o=job.out.getContext('2d');o.clearRect(0,0,1024,512);o.globalAlpha=.82+wet*.18;o.drawImage(job.temp,0,0);o.globalAlpha=1;job.texture.needsUpdate=true}
